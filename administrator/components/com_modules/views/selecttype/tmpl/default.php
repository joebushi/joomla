...
for ($i=0; $i < $count; $i++) {
	...
	$link = 'index.php?option=com_modules&amp;task=module.edit&amp;module='. $row->module .'&amp;created=1&amp;client='. $this->client->id;
	if (!$k) {
		?>
		...
		<td width="50%">
			<span class="editlinktip hasTip" title="<?php echo JText::_(stripslashes($row->name)).' :: '.JText::_(stripslashes($row->descrip)); ?>">
				<input type="radio" name="module" value="<?php echo $row->module; ?>" id="cb<?php echo $i; ?>" onclick="isChecked(this.checked);">
