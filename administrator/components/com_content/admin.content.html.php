<?php
/**
* @version $Id: admin.content.html.php,v 1.7 2005/09/03 21:04:59 alekandreev Exp $
* @package Mambo
* @subpackage Content
			...
			<td valign="top" width="40%">
				...
				<tr>
					...
					?>
						...
						<tr>
							<td valign="top" align="right">
								<?php echo $_LANG->_('Save as Draft'); ?>:
							</td>
							<td>
								<input type="radio" name="draft" checked="checked" value="no" id="newrevoff"/><label for="newrevoff">No</label>&nbsp;&nbsp;&nbsp;
								<input type="radio" name="draft" value="yes" id="newrevon"/><label for="newrevon">Yes</label>
							</td>
						</tr>
						<tr>
							<td valign="top" align="right">
					...
					$title = $_LANG->_( 'Media' );
					$tabs->endTab();
					$tabs->startTab( $title, "media-page" );
									
					?>
						<table class="adminform" width="100%">
						<tr>
							<td>
								<?php echo $_LANG->_( "Media handler" ); ?>
							</td>
						</tr>
						<tr>
							<td>
							<?php 
								mosFS::load( '#mambo.mediamanager.mediahandler' );				
								$media = new mediaHandler;
								echo $media->showMediaList();
							?>
							</td>
						</tr>
						</table>
					<?php		
					$title = $_LANG->_('Revisions');
					$tabs->endTab();
					if ($row->id) { // we don't want the revisions tab if this is a new item
						$tabs->startTab($title, "revisions-page");
					?>
						<table class="adminform">
						<tr><th>Item Revisions</th></tr>
						<tr>
							<td>
							<input type="hidden" name="newRevision" value="-1"/>
								<table width="100%">
								<?php
									foreach($row->listRevisions() as $revision) { ?>
										<tr<?php if ($revision['active']) echo ' style="background: #E9EFF5"'; ?>>
										<td><b><?php echo $revision['revision']; ?></b></td>
										<td>by <?php echo $revision['name']; ?> on <?php echo(strftime('%m/%d/%y %H:%M',$revision['modified'])); ?></td>
										<?php if($revision['active']) { ?>
										<td colspan="3">Active</td>
										<?php } else { ?>
										<td><a href="#" onclick="goDoTask(this,'submit-revision','task=setrevision,revision_id=<?php echo($revision['revision']);?>');">Make Active</a></td>
										<td><a target="_new" href="<?php echo "$mosConfig_live_site/index.php?option=com_content&task=view&id={$row->id}&showrev={$revision['revision']}";?>">View</a></td>
										<td><a target="_new" href="<?php echo "$mosConfig_live_site/index.php?option=com_content&task=view&id={$row->id}&showdiff={$revision['revision']}";?>">Diff</a></td>
										<?php } ?>
								<?php } ?>
								</table>
							</td>
