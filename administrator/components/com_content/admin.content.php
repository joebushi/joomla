<?php
/**
* @version $Id: admin.content.php,v 1.6 2005/09/02 19:07:44 alekandreev Exp $
* @package Mambo
* @subpackage Content
...
function trashDeleteConfirm() {
	...
	if ( $cids ) {
		...
		. "\n FROM #__content AS a"
		. "\n WHERE ( a.id IN ( $cids ) ) AND a.active = 1"
		. "\n ORDER BY a.title"
		;
		$database->setQuery( $query );
...
function viewArchive( $sectionid, $option ) {
	...
	. "\n INNER JOIN #__sections AS s ON s.id = c.sectionid"
	. "\n LEFT JOIN #__users AS u ON u.id = c.created_by"
	. "\n WHERE c.state = '-1' AND c.active = 1"
	. "\n GROUP BY u.name"
	. "\n ORDER BY u.name"
	;
	$authors[] = mosHTML::makeOption( '0', '- ' . $_LANG->_( 'Author' ) . ' -' );
...
function editContent( $uid=0, $sectionid=0, $option ) {
	...
	if ( $sectionid == 0 ) {
		...
		$lists['sectionid'] = mosHTML::selectList( $php51fix = $database->loadObjectList(), 'sectionid', 'class="inputbox" size="1" '. $javascript, 'value', 'text', intval( $row->sectionid) );
	}
...
/**
* Changes the active revision for the item
*/
function setRevision() {
	...
	$newrev = mosGetParam($_POST, 'newRevision', 0);
	$row->setActiveRevision($newrev, $id);
	mosRedirect( 'index2.php?option=com_content&sectionid='. $sectionid .'&task=edit&id='. $id);
}
...
function copyItemSave( $cid, $sectionid, $option ) {
	...
	for ( $i = 0; $i < $total; $i++ ) {
		...
		. "\n WHERE a.active = 1 AND a.id = ". $cid[$i]
		;
		$database->setQuery( $query );
