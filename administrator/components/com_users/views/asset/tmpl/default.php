...
<div style="float: right">
				<button type="button" onclick="Joomla.submitform('asset.saveAccess', this.form);window.top.setTimeout('window.parent.document.getElementById(\'sbox-window\').close()', 700);">
...
foreach($this->usergroups as $usergroup)
{
	...
	foreach($this->items->children() as $item)
	{
		if($item->name() == 'asset')
		{
			...
			{
				echo'<tr><td>'.$item2->attributes('name').'</td>';
				echo '<td>'.JHTML::_('select.booleanlist', JRequest::getVar('component').'['.$usergroup->title.']['.$item2->attributes('value').']').'</td></tr>';
			}
		}
	}
