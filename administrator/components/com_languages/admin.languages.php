...
switch ($task) {
	case 'new':
		mosRedirect( 'administrator/index2.php?option=com_installer&element=language' );
		break;
...
function publishLanguage( $p_lname, $option ) {
	...
	if ($fp = fopen("../configuration.php","w")){
		...
		fclose($fp);
		mosRedirect("administrator/index2.php","Configuration succesfully updated!");
	} else {
		mosRedirect("administrator/index2.php","Error! Make sure that configuration.php is writeable.");
	}
...
function removeLanguage( $cid, $option, $client ) {
	...
	*/
	...
	mosRedirect( 'administrator/index2.php?option=com_installer&element=language&client='. $client .'&task=remove&cid[]='. $cid );
...
function editLanguageSource( $p_lname, $option) {
	...
	if ($fp = fopen( $file, "r" )) {
		...
		mosRedirect( "administrator/index2.php?option=$option&mosmsg=Operation Failed: Could not open $file" );
	}
}
...
function saveLanguageSource( $option ) {
	...
	if (!$language) {
		mosRedirect( "administrator/index2.php?option=$option&mosmsg=Operation failed: No language specified." );
	}
	if (!$filecontent) {
		mosRedirect( "administrator/index2.php?option=$option&mosmsg=Operation failed: Content empty." );
	}
	...
	if (is_writable( $file ) == false) {
		mosRedirect( "administrator/index2.php?option=$option&mosmsg=Operation failed: The file is not writable." );
	}
	...
	if ($fp = fopen ($file, "w")) {
		...
		mosRedirect( "administrator/index2.php?option=$option&mosmsg=Operation failed: Failed to open file for writing." );
	}
}
?>