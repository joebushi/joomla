...
function saveMambot( $option, $client, $task ) {
	...
	switch ( $task ) {
		case 'apply':
			...
			mosRedirect( 'administrator/index2.php?option='. $option .'&client='. $client .'&task=editA&hidemainmenu=1&id='. $row->id, $msg );

		case 'save':
		default:
			$msg = 'Successfully Saved Mambot: '. $row->name;
			mosRedirect( 'administrator/index2.php?option='. $option .'&client='. $client, $msg );
			break;
	}
}
...
function removeMambot( &$cid, $option, $client ) {
	...
	mosRedirect( 'administrator/index2.php?option=com_installer&element=mambot&client='. $client .'&task=remove&cid[]='. $cid[0] );
}
...
function saveOrder( &$cid ) {
	...
	mosRedirect( 'administrator/index2.php?option=com_mambots', $msg );
} // saveOrder
?>