...
?>
<form action="<?php echo JRoute::_('index.php?option=com_members&view=members&layout=modal&tmpl=component&field='.JRequest::getCmd('field'));?>" method="post" name="adminForm">
	...
	<table class="adminlist">
		<thead>
			<tr>
				...
				<th nowrap="nowrap" width="25%">
					<?php echo JHtml::_('grid.sort', 'Members_Heading_UserName', 'a.username', $this->state->get('list.direction'), $this->state->get('list.ordering')); ?>
				</th>
				<th nowrap="nowrap" width="25%">
					<?php echo JHtml::_('grid.sort', 'Members_Heading_Groups', 'a.group_names', $this->state->get('list.direction'), $this->state->get('list.ordering')); ?>
				</th>
			</tr>
		</thead>
		<tfoot>
		...
		<?php
			...
			<tr class="row<?php echo $i % 2; ?>">
				<td>
					<a style="cursor: pointer;" onclick="window.parent.jxSelectMember('<?php echo $item->id; ?>', '<?php echo str_replace(array("'", "\""), array("\\'", ""),$item->username); ?>', '<?php echo JRequest::getCmd('field'); ?>');">
