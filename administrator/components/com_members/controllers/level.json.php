...
class MembersControllerLevel extends JController
{
	...
	public function sendResponse($body)
	{
		...
		echo json_encode(new MembersLevelResponse($body));
...
/**
 * Members Access Level JSON Response Class
 *
 * @package		Joomla.Administrator
 * @subpackage	com_members
 * @since		1.6
...
class MembersLevelResponse
{
	public function __construct($state)
	{
		...
		if (JError::isError($state))
		{
			...
			$this->error	= true;
			$this->header	= JText::_('MEMBERS_LEVEL_HEADER_ERROR');
...
JError::setErrorHandling(E_ALL, 'callback', array('MembersControllerLevel', 'sendResponse'));