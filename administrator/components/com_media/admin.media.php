...
if (is_int(strpos ($listdir, "..")) && $listdir<>'') {
	mosRedirect( "administrator/index2.php?option=com_media&listdir=".$_POST['dirPath'], "NO HACKING PLEASE" );
}
...
switch ($task) {
	...
	case 'newdir':
		if (ini_get('safe_mode')=='On') {
			mosRedirect( "administrator/index2.php?option=com_media&listdir=".$_POST['dirPath'], "Directory creation not allowed while running in SAFE MODE as this can cause problems." );
			}
		else {
...
function create_folder($folder_name,$dirPath) {
	...
	if(strlen($folder_name) >0) {
		if (eregi("[^0-9a-zA-Z_]", $folder_name)) {
			mosRedirect( "administrator/index2.php?option=com_media&listdir=".$_POST['dirPath'], "Directory name must only contain alphanumeric characters and no spaces please." );
		}
...
function do_upload($file, $dest_dir) {
	global $clearUploads;
		...
		if (file_exists($dest_dir.$file['name'])) {
			mosRedirect( "administrator/index2.php?option=com_media&listdir=".$_POST['dirPath'], "Upload FAILED.File allready exists" );
		}
        ...
        if(!$noMatch){
			mosRedirect( "administrator/index2.php?option=com_media&listdir=".$_POST['dirPath'], 'This file type is not supported' );
        }

		if (!move_uploaded_file($file['tmp_name'], $dest_dir.strtolower($file['name']))){
			mosRedirect( "administrator/index2.php?option=com_media&listdir=".$_POST['dirPath'], "Upload FAILED" );
			}
		else {
			mosChmod($dest_dir.strtolower($file['name']));
			mosRedirect( "administrator/index2.php?option=com_media&listdir=".$_POST['dirPath'], "Upload complete" );
		}
