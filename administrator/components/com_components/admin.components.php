<?php
/**
 * @version $Id: admin.components.php,v 1.3 2005/08/28 06:35:57 pasamio Exp $
 * @package Mambo
...
class componentTasks extends mosAbstractTasker {
	...
	function componentTasks() {
		...
		$this->registerTask( 'installFromUrl', 'install' );
	}
	...
	function install() {
		...
		$url = mosGetParam( $_REQUEST, 'userfile', null );
		...
		switch($this->getTask()) {
			case 'installUpload':
				...
				break;
			case 'installFromUrl':
				$webinstaller = new mosInstallerFactory();
				$webinstaller->webInstall( $url );
				break;
			default: 
				$installer->installDir( $userfile );
				break;
		}
		if($this->getTask() != 'installFromUrl') {
			...
			componentScreens::installDone( $webinstaller->getClass() );
		}		
	}
