<?php
/**
* @version $Id: mod_fullmenu.php,v 1.2 2005/08/29 15:52:19 alekandreev Exp $
* @package Mambo
...
class mosFullAdminMenu {
	...
	*/
	function showMenu($usertype='', $disabled = false) {
		...
		if( !$disabled ) {
			...
			if( $cache->_caching ) {
				$cache->callId( "mosFullAdminMenu::_showDisabled", array($usertype), 'disabled.js' );
			} else {
	...
	*/
	function _showEnabled( $usertype='' ) {
	   ...
	   . "\n WHERE a.scope = 'content' AND b.active = 1"
	   . "\n GROUP BY a.id"
	   . "\n ORDER BY a.ordering"
	   ;
