<?php
/**
* @version $Id: installer.php,v 1.4 2005/08/30 14:42:59 pasamio Exp $
* @package Mambo
...
class mosInstaller extends mosAbstractLog {
	...
	*/
	...
	function downloadPackage($url,$target=0) {
		global $mosConfig_cachepath,$_LANG,$mainframe;
		$target = false;
		// Check that the zlib is available
		...
		if(!$target) {
			$target = $mosConfig_cachepath . '/' . $this->getFilenameFromURL($url);
		}			
	...
	*/
	 ...
	 */
		function install( $installDirectory='' ) {
			...
			$checks = $_MAMBOTS->trigger( 'onBeforeInstall', $this->i_installdir );
	...
	function uninstall( $name, $client=0 ) {
		...
		$error_msg = '';
		$_MAMBOTS->loadBotGroup('system');
		$checks = $_MAMBOTS->trigger( 'onBeforeUninstall', array($name, $this->elementType) );
		foreach ($checks as $check) {
			if (is_a( $check, 'patError' )) {
				$error = 1;
				$error_msg .= $check->getMessage();
