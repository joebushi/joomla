<?php
/**
* @version $Id: client.php,v 1.1 2005/08/28 15:43:52 pasamio Exp $
...
// xml rpc server
/*
Use to store an xml rpc server
*/
class update_xmlrpcserver {
	var $host = '';		// TEXT(0)
	var $path = '';		// TEXT(0)
	function update_xmlrpcserver($host,$path) {
		$this->host = $host;
		$this->path = $path;
	}
}
...
class updatedependency extends mosDBTable {
	...
	function updatedependency( &$db ) {
		$this->mosDBTable('#__update_dependency', 'dependencyid', $db);
	}
}
...
class updaterelease extends mosDBTable {
	...
	function updaterelease( &$db ) {
		$this->mosDBTable('#__update_release', 'releaseid', $db);
	}
}
