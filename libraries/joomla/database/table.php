...
abstract class JTable extends JObject
{
	...
	*/
	...
	/**
	 * Abstract method to return the asset to inherit from. In case of an
	 * article, this would return the id of the category that article is in.
	 * 
	 * @return 	string
	 * @since	1.6
	 */
	public function getAssetParent()
	{
		return false;
	}
	...
	public function store($updateNulls = false)
	{
		 ...
		 */
		if($this->getAssetParent() && JRequest::getvar('inherit', 1))
		{
			$query = 'SELECT id '
					.'FROM #__access_assets '
					.'WHERE section='.$this->_db->Quote($this->getAssetSection())
					.' AND name='.$this->_db->Quote($this->getAssetParent());
			$this->_db->setQuery($query);
			$parentid = $this->_db->loadResult();
		} else {
			$parentid = 0;
		}
		$query = 'SELECT * FROM #__access_assets '
				.'WHERE section='.$this->_db->Quote($this->getAssetSection())
				.' AND name='.$this->_db->Quote($this->getAssetNamePrefix());
		$this->_db->setQuery($query);
		$asset = $this->_db->loadObject();
		if(!is_object($asset))
		{
			$asset = new stdClass();
			$asset->id = null;
		...
		if($oldparent != $asset->parent)
		{
			$query = 'SELECT id FROM #__access_assets WHERE parent = '.$asset->id;
			$this->_db->setQuery($query);
			$result = $this->_db->loadResultArray();
			$childassets[] = $result;
			while(count($result))
			{
				$query = 'SELECT id FROM #__access_assets '
						.'WHERE parent IN ('.implode(',', $childassets[count($childassets) - 1]).')';
				$this->_db->setQuery($query);
				$result = $this->_db->loadResultArray();
				$childassets[] = $result;
			}
			$result = array();
			foreach($childassets as $childs)
			{
			...
			if($asset->parent == 0)
			{
				$query = 'SELECT rule_id FROM #__access_asset_rule_map WHERE asset_id = '.$asset->id;
				$this->_db->setQuery($query);
				$rules = $this->_db->loadResultArray();
				if(count($rules))
				{
					$query = 'DELETE FROM #__access_rules WHERE id IN ('.implode(',', $rules).');';
					$query .= 'DELETE FROM #__usergroups_rule_map WHERE rule_id IN ('.implode(',', $rules).');';
					$query .= 'DELETE FROM #__access_action_rule_map WHERE rule_id IN ('.implode(',', $rules).');';
					$this->_db->setQuery($query);
					$this->_db->Query();
				}
			}
			$query = 'DELETE FROM #__access_asset_rule_map WHERE asset_id IN ('.implode(',', $childassets).')';
			$this->_db->setQuery($query);
			$this->_db->Query();
			if($asset->parent != 0 && JRequest::getVar('inherit', 1))
			{
				$query = 'SELECT rule_id FROM #__access_asset_rule_map WHERE asset_id = '.$asset->parent;
				$this->_db->setQuery($query);
				$rules = $this->_db->loadResultArray();
