...
class JElementAccess extends JElement
{
	...
	*/
	...
	function fetchElement($name, $value, &$node, $control_name)
	{
		...
		$extension = $node->attributes('extension', 'com_content');

		$aclContent =& JAuthorizationContentItem::getInstance();
		$aclContent->load();
		$contentItems = $aclContent->getContentItems($extension);

		return JHTML::_('select.genericlist',  $contentItems, ''.$control_name.'['.$name.']', $class, 'value', 'name', $value, $control_name.$name);
	}
}
