<?php
/**
* @version $Id: CHANGELOG.php,v 1.11 2005/08/31 17:31:34 facedancer Exp $
* @package Mambo
...
--------------------------------------------------------------------------------------------------------------
31-Aug-2005 Mateusz Krzeszowiec
 + Added 'move' files and folders in Media Manager
 + Added general 'archive' icons and filetype in Media Manager
 + Added extraction option in icons view 
 # Fixed media manager and media handler, now if you'll set no root dir in media manager config images directory will be showed by default
 # Fixed delete function in Media Manager
 # Fixed bug that allowed viewing files and folders above root directory (Media Manager)
 # Fixed switching to default view after pressing 'Save' in Media Manager configuration
 ! You can move directories into *mambo* root directory but not files, also directories and files will NOT be overwritten

31-Aug-2005 Samuel Moffatt
 ! Dependency Checker is still very much in a proof of concept stage. Whilst the system appears to demonstrate the capability to halt component install or uninstall, its ability across other subsystems (e.g. mambots) appears inactive.

29-Aug-2005 Alek Andreev
 ^ All SELECT queries involving mos_content now also have an "active = 1" condition. Please make sure new queries do that, too.
 + mosDBVersionedTable class in includes/versionining.php
 + Changing content now creates a new version each time. Currently, the latest version is made active. There will be an option to disable that soon.
 ^ mos_content primary key now on id and version. Added 'active' column to mos_content. The install and update SQLs have been updated accordingly.
 # Fixed 2 pages in admin to work properly with PHP 5.1.
 # Fixed duplicated IDs for newsfeeds in the sample data.
 
28-Aug-2005 Mateusz Krzeszowiec
 + Added new way of putting images and files into content - media handler library (supported editors:  no wysiwyg, tinymce, mosce, wysiwygpro, tmedit (partial))
 + Added root directories choice in media manager configuration
 + Added choice to thumbnail (or not) onsite and offsite images in mosimage configuration
 ^ Changed default com_media and MOS Image configuration
 + Media tab (media handler) now avaliable when editing static content 

29-Aug-2005 Samuel Moffatt
 + Added Update System Cache Rebuild Mambot
 ^ Shifted a portion of the Mambo Update Client into the libraries tree
 
28-Aug-2005 Samuel Moffatt
 + Mambo Update Client test functional
 ! Hack required to get update client to work requires global CMT install check to be disabled. Other alterations need to be made first.

27-Aug-2005 Samuel Moffatt
 ^ Switched to library loaders for code
 - Original installer class files
 # Fixed up issue with XML install files in DomIT!
 + Added library installer library
 ^ Made the TODO.php file correct
 # Fixed up HTTP_ACCEPT_LANG in installer
 # Fixed up missing / in administator title icons

14-Aug-2005 Mateusz Krzeszowiec
 + Added archives extraction (zip only for now)
 + Added massive upload 

11-Aug-2005 Mateusz Krzeszowiec
 + Added details view in Media Manager
 + Added configuration for Media Manager
 + Added thumbnailing in Media Manager
 + Added phpThumb
 + Added phpThumb cache in /cache/phpThumb/
 + Added thumbnailing in MOS Image
 + Added MOS Image configuration
 # Fixed upload problems in Media Manager
 # Fixed folder creation in Media Manager
...
10-Aug-2005 Andrew Eddie
 ...
 ^ Changed INSTALL to INSTALL.php to reduce visible element to determine the Mambo version
...
14-Jul-2005 Emir Sakic
 # Fixed [#5952] Mambo does a 302 redirect not 301
...
13-Jun-2005 Andrew Eddie
 ...
 + addToHead Mambo macro in patTemplate
...
21-April-2005 Rey Gigataras
 ...
 # Fixed [#5799] Old image header_blue.jpg in Mambo 4.5.3 installer
...
05-April-2005 Emir Sakic
 # Fixed [#3149] Error when creating mambo for search
...
22-Feb-2005 Rey Gigataras
 ...
 # Fixed [#4635] Mambo 4.5.2.1 Newsfeed component generates image tag instead of img tag
...
------------------------------------------------------------------
 ...
 This has been added to make it easier to remove the mambo copyright statement without having to worry about breaching the GPL
...
This depreciation was made as the separation of popup handling into separate files in the /popup directory is legacy from Mambo 4.0
...
1. We moved all cache classes into /includes/mambo.cache.php
  ...
  This class will take a string and cache it as a file in the global cache.
  The code returns a path or url to the cached file depentent on the 
  method used.

------------------------------------------------------------------

+ Added new Script Handler
  
  A Script Handler is accessed using mosFactory::getScriptHandler()
  
  $scriptHandler =& mosFactory::getScriptHandler()
  $scriptHandler->add( 'http://www.somehost.com/script.js', 'http' );
  or
  $scriptHandler->add( 'http://www.somehost.com/style.css', 'http' );
  or
  $scriptHandler->add( 'js script', 'js', true ); // the last argument allows for caching
  or
  $scriptHandler->add( 'my js script', 'css', false ); // the last argument allows for caching
  
  When a template is loaded Scripts added as above will automaticaly be included in the output
  
  NOTE: Full functionality is dependent on additional work.
  
 ------------------------------------------------------------------
 
 # Fixed administrator menu items displaying when menu disabled

  ------------------------------------------------------------------
 
+ Added a site Initializer
  
  The Initialiser is in two parts, an administration Component and a Mambot
  
  The Mambot retrieves the browser type of the client and then searches
  for a match in the provided ini file. If a match is found either the Template
  is changed or the client is redirected to the given site.
  
  The Initializer Manager is used to create and edit the ini file used by the
  Mambot. It is important to note that a file has been used rather than a database
  table for speed.
