 ...
 *
 * $Id: File.php,v 1.1 2005/08/25 14:21:18 johanjanssens Exp $
 *
 * @package		patTemplate
 * @subpackage	Readers
	...
	*/	
	function _resolveFullPath( $filename, $relativeTo = false )
	{
		if (preg_match( '/^[a-z]+:\/\//', $filename )) {
		    ...
		    if (!is_array($this->_options['root'])) {
                $this->_options['root'] = array($this->_options['root']);
		    }
		    foreach ($this->_options['root'] as $root) {
    		    if ($relativeTo === false) {
        			$baseDir = $root;
    		    } else {
