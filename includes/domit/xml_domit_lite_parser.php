...
define ('DOMIT_LITE_VERSION', '0.19');
...
class DOMIT_Lite_Document extends DOMIT_Lite_ChildNodes_Interface {
	...
	*/
	function toString($htmlSafe = false, $subEntities = true) {
		$result = '';
		$total = $this->childCount;
...
class DOMIT_Lite_Element extends DOMIT_Lite_ChildNodes_Interface {
	...
	*/
	function getNamedElements(&$nodeList, $tagName) {
		...
		$total = $this->childCount;			
		for ($i = 0; $i < $total; $i++) {
			if(is_object($this->childNodes[$i])) { 
	...
	*/
	function toString($htmlSafe = false, $subEntities = true) {
		...
		foreach ($this->attributes as $key => $value) {
			$result .= ' ' . $key . '="';
			$result .= ($subEntities && !$htmlSafe) ? DOMIT_Utilities::convertEntities($value,
...
class DOMIT_Lite_TextNode extends DOMIT_Lite_Node {
	...
	*/
	function toString($htmlSafe = false, $subEntities = true) {
		...
		$result = ($subEntities && !$htmlSafe) ? DOMIT_Utilities::convertEntities($this->nodeValue,
...
class DOMIT_Lite_CDATASection extends DOMIT_Lite_TextNode {
	...
	*/
	function toString($htmlSafe = false, $subEntities = true) {
		$result = '<![CDATA[';
		$result .= ($subEntities && !$htmlSafe) ? str_replace("]]>", "]]&gt;", $this->nodeValue) :
								 $this->nodeValue;
		$result .= ']]>';
