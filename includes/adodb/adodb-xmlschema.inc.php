 ...
 * XML schema.
 *
 * Last Editor: $Author: johanjanssens $
...
class dbData extends dbObject {
	...
	*/
	function create( &$xmls ) {
		$table = $xmls->dict->TableName($this->parent->name);echo "<br>$table";
		...
		// <mamboHack>
		$identityInsert = (count( $this->data ) > 0 && $xmls->db->databaseType == 'mssql');
		if ($identityInsert) {
		...
		foreach( $this->data as $row ) {
			...
			foreach( $row as $field_id => $field_data ) {
				...
				switch( $table_fields[$field_id]['TYPE'] ) {
					case 'T':
						// MAMBO
						$fields[$name] = $xmls->dict->DateTimeInsert( $field_data );
						break;
					case 'I':
					case 'I1':
					case 'I2':
...
class adoSchema {
	...
	* @param array Optional array of table names to process (ADDED FOR MAMBO)
	* @return string Generated XML schema
	*/
	function ExtractSchema( $data = FALSE, $tables = null ) {
		$old_mode = $this->db->SetFetchMode( ADODB_FETCH_NUM );
		
		// CUSTOMISATION FOR MAMBO www.mamboserver.com
		if (is_null( $tables )) {
