<?php
/**
* @version $Id: content.item.php,v 1.7 2005/09/03 06:46:57 alekandreev Exp $
* @package Mambo
* @subpackage Content
...
class comContentItem {
	...
	function showItemMain( $uid, $gid, &$access, $pop, $option )
	{
		...
		if ($_COOKIE[md5($mosConfig_live_site)]) {
			session_name( md5( $mosConfig_live_site ) );
			session_start();
			$authenticated = !empty($_SESSION['session_userstate']);
		}
		...
		if (isset($_GET['showrev']) and $authenticated)
			$query .= " AND a.revision = '{$_GET['showrev']}'";
		else 
		...
		if ( $database->loadObject( $row ) ) {
			...
			if ( $params->get( 'item_navigation' ) ) {
				...
				. "\n AND a.active = 1 AND a.state = $row->state AND ordering < $row->ordering"
				. ( $access->canEdit ? '' : "\n AND a.access <= '$gid'" )
				...
				$query = "SELECT a.id"
				. "\n FROM #__content AS a"
				. "\n WHERE a.active = 1 AND a.catid = $row->catid"
			...
			// if the user wants a diff, do a diff
			if (isset($_GET['showdiff']) and $authenticated) {
				require_once("includes/diff.php");
				$otherRev = new mosContent($database);
				$otherRev->load($uid,$_GET['showdiff']);
				$fieldsToDiff = array('title','title_alias','introtext','fulltext');
				foreach($fieldsToDiff as $field)
					$row->$field = diff($row->$field, $otherRev->$field);
				$mainframe->setPageTitle($row->title . " (diff with revision {$_GET['showdiff']})");
			} else if (isset($_GET['showrev']) and $authenticated) {
				$mainframe->setPageTitle($row->title . " (showing revision {$row->revision})");
			} else {
	...
	function editItem( $uid, $gid, &$access, $sectionid=0, $task, $Itemid )
	{
		...
		. "\n FROM #__content"
		. "\n WHERE active = 1 AND catid = '$row->catid'"
		. "\n ORDER BY ordering"
		;
	...
	*/
	...
	function findKeyItem( $gid, $access, $pop, $option )
	{
		...
		$query = 'SELECT id
			FROM #__content
			WHERE attribs LIKE \'%keyref=' . $keyref . '%\' AND active = 1
			';
		$database->setQuery( $query );
