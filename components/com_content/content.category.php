<?php
/**
* @version $Id: content.category.php,v 1.3 2005/08/29 21:40:58 alekandreev Exp $
* @package Mambo
* @subpackage Content
...
class comContentCategory {
	...
	*/
	function showCategory( $id, $gid, &$access, $sectionid, $limit, $limitstart, $task  )
	{
		...
		. $xwhere2
		. ( $noauth ? "\n AND b.access <= '$gid'" : '' )
		. "\n WHERE c.section = '$category->section' AND b.active = 1"
		. $xwhere
		. ( $noauth ? "\n AND c.access <= '$gid'" : '' )
		. "\n GROUP BY c.id"
		. $empty
		...
		. $join
		. "\n WHERE a.catid = '$category->id' AND a.active = 1"
		. $xwhere
		. ( $noauth ? "\n AND a.access <= '$gid'" : '' )
	...
	function showBlogCategory( $id=0, $gid, &$access, $pop )
	{
		...
		. "\n LEFT JOIN #__groups AS g ON a.access = g.id"
		. $where
		. "\n AND a.active = 1 AND s.access <= ". $gid
